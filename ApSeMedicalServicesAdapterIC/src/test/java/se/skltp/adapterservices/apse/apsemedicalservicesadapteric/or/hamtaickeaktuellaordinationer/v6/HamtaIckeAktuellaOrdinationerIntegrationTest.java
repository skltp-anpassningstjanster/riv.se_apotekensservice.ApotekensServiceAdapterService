...
public class HamtaIckeAktuellaOrdinationerIntegrationTest extends AbstractTestCase {

	@SuppressWarnings("unused")
	private static final Logger log = LoggerFactory.getLogger(
      HamtaIckeAktuellaOrdinationerIntegrationTest.class);

	private static final String DEFAULT_SERVICE_ADDRESS = getAddress("inbound.endpoint.http.apotekensservice.or.HamtaIckeAktuellaOrdinationer.v6");
	private static final String ERROR_LOG_QUEUE = "SOITOOLKIT.LOG.ERROR";
	private AbstractJmsTestUtil jmsUtil = null;

	public HamtaIckeAktuellaOrdinationerIntegrationTest() {
	...
	protected String getConfigResources() {
		return "soitoolkit-mule-jms-connector-activemq-embedded.xml," + "ApSeMedicalServicesAdapterIC-common.xml,"
				+ "services/OR-HamtaIckeAktuellaOrdinationer-v6-apse-service.xml,"
				+ "teststub-services/OR-HamtaIckeAktuellaOrdinationer-v6-apse-teststub-service.xml";
	}
	...
	public void testRequestSsnWithCompleteArgosHeader() throws Exception {
		...
		HamtaIckeAktuellaOrdinationerResponseType response = new HamtaIckeAktuellaOrdinationerTestConsumer(
				DEFAULT_SERVICE_ADDRESS).requestIncludingCompleteArgosInformation(ssn, to);

		assertNotNull(response);
		assertEquals(ssn, response.getOrdinationKortInfolista().getPersonnummer());
	}
	...
	public void testRequestWithEncoding() throws Exception {
		...
		assertNotNull(response);
		assertEquals("ÖÄÅ", response.getOrdinationKortInfolista().getPersonnummer());
	}
	...
	public void testSoapFaultIsReturnedWhenProducerThrowsRuntimeException() throws Exception {
		...
		try {
			new HamtaIckeAktuellaOrdinationerTestConsumer(DEFAULT_SERVICE_ADDRESS)
